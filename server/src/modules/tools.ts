...
export const TOOLS: ToolDef[] = [
  ...
  { id: 'gitea', name: 'Gitea(本地仓库)', category: 'devops', port: 3000, install: 'docker run -p 3000:3000 -p 2222:22 gitea/gitea', ui: 'dashboard', docs: '轻量 Git 托管服务,本地代码仓库(HTTP/SSH)' },
  { id: 'sonarqube', name: 'SonarQube', category: 'devops', port: 9000, install: 'docker run sonarqube:lts-community', ui: 'dashboard' },
  { id: 'sonar-scanner', name: 'SonarScanner(扫描器)', category: 'devops', install: 'docker run sonarsource/sonar-scanner-cli', ui: 'none', docs: '代码静态分析扫描器,配合 SonarQube 使用' },
  { id: 'nexus', name: 'Nexus(制品仓库)', category: 'devops', port: 8081, install: 'docker run -p 8081:8081 sonatype/nexus3', ui: 'dashboard', docs: 'Maven/npm/Docker/raw 制品仓库' },
...
export const DOCKER_TEMPLATE: Record<string, DockerTemplate> = {
  ...
  sonarqube:  { image: 'sonarqube:lts-community', ports: [9000], extra: '-v sonarqube-data:/opt/sonarqube/data -v sonarqube-logs:/opt/sonarqube/logs -v sonarqube-extensions:/opt/sonarqube/extensions' },
  ...
  jenkins:    { image: 'jenkins/jenkins:lts', ports: [8080], extra: '-p 50000:50000 -v jenkins-data:/var/jenkins_home' },
  gitea:      { image: 'gitea/gitea:1.22', ports: [3000], extra: '-p 2222:22 -v gitea-data:/data' },
  'sonar-scanner': { image: 'sonarsource/sonar-scanner-cli:5.0.1', ports: [] },
  nexus:      { image: 'sonatype/nexus3:3.69.0', ports: [8081], extra: '-v nexus-data:/nexus-data' },
  // ===== 安全 / 渗透(一键拉起靶场与工具) =====
